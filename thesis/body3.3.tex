...
% We take \(L=100\), \(c=0.5\), and \(x_0=30\). Since the domain is large relative to the soliton width, the real-line profile is exponentially small at the periodic seam. We therefore use its restriction to \((0,L)\) as an effectively periodic reference profile and neglect the resulting exponentially small mismatch. The amplitude is one and the propagation speed is
% \[
% s=\frac{1}{1-c^2}=\frac43.
% \]
...
\begin{equation}
  ...
  \label{eq:bbm-sensor}
\end{equation}
It measures the wave in a narrow neighbourhood of its predicted terminal position and is therefore sensitive to phase, amplitude, and shape errors near the sensor. High-order quadrature of this reference profile gives
\begin{equation}
...
This subsection identifies the additional support entity. 
% The implementation continues to evaluate and project the original weak residual and does not insert a case-specific strong-residual formula.
Let
\begin{equation}
  j_n=u_h(t_{n-1}^{+})-P_{n-1}u_h(t_{n-1}^{-})
  \label{eq:bbm-discrete-jump}
\end{equation}
be the dG interface defect, including the transfer $P_{n-1}$ from the preceding slab when consecutive slabs use different spatial meshes. For the spatial $L^2$ mass pairing used by the heat and standard advection--diffusion problems, the dG jump contributes
\begin{equation}
...
\end{equation}
The first sum is supported on $K\times\{t_{n-1}\}$, the interior of a temporal face. The second is supported on \( F\times\{t_{n-1}\},\)
...
For the present cG(1) discretisation, $j_n$ is piecewise linear. Therefore
\begin{equation*}
...
The global estimator uses the practical nonlinear DWR estimator~\eqref{eq:practical-nonlinear-estimator}. The low-order and enriched spaces are, respectively,
\begin{equation*}
...
\end{equation*}
We take the D\"orfler marking fraction as $0.35$ and bisect a time slab when at least $10\%$ of its spatial cells are selected. We initially use $50$ spatial cells and $6$ time slabs. The terminal time is set to be $T = 18$.
...
$N_t$ & Terminal DoFs & $ J_{\mathrm{ref}}-J(u_h)$ & $\eta_{\mathrm{global}}$ & $I_{\mathrm{eff}}^{\mathrm{glob}}$ \\
\midrule
