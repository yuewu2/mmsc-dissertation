...
We choose the squared space--time \(L^2\) error as the goal functional:
\begin{equation}
  J(v)
  \coloneqq
...
\end{equation*}
Accordingly, the adjoint problem linearised at $u_h$ is
\begin{equation}
  -\partial_t z-\Delta z
  =
  u_h-u_{\mathrm{ex}}
...
For comparison, we derive a classical non-negative marking score from a manually derived strong-residual representation of the heat equation \cite{Bangerth_Rannacher_2003,Becker_Rannacher_2001}. After integrating by parts, the cell residual on a space--time cell \(Q_{K,n}=K\times I_n\) is
\begin{equation}
  R_{K,n}
  \coloneqq
  f+\Delta u_h-\partial_t u_h.
  \label{eq:mh-strong-cell-residual}
\end{equation}
Let \(\mathcal F_h^{\mathrm{int}}(K)\) denote the set of interior spatial facets of \(K\). For \(F\in\mathcal F_h^{\mathrm{int}}(K)\), define
\begin{equation}
  r_{K,F,n}
  \coloneqq
  \frac12
  \llbracket \nabla u_h\cdot n_K \rrbracket.
...
\begin{equation}
  ...
  -
  \sum_{F\in\mathcal F_h^{\mathrm{int}}(K)}
...
We apply this action to the dual weight \(e_z=z_h^+-z_h\). The Cauchy--Schwarz inequality bounds the volume, spatial-facet, and temporal-interface terms separately. 
...
\zeta_{K,n}^{\mathrm{s}}
\coloneqq{}&
\lVert e_z\rVert_{K\times I_n}
+
h_K^{1/2}
\left(
\sum_{F\subset\partial K}
\lVert e_z\rVert_{F\times I_n}^{2}
\right)^{1/2},
...
k_n^{-1/2}\lVert [u_h]_{n-1}\rVert_K,\\
  \zeta_{K,n}^{\mathrm{t}}
  \coloneqq{}&
k_n^{1/2}\lVert e_z(t_{n-1}^{+})\rVert_K.
...
\end{equation}
It provides a non-negative measure of the goal error associated with each space--time cell. The sum of the local magnitudes is generally no smaller than the magnitude of the corresponding signed global estimator, with equality only when no cancellation occurs.


\paragraph{Automated bubble--cone localisation.}
The automated method instead reconstructs the weak residual directly from its action on local bubble and cone test functions. For the present heat discretisation, the recovered residual has three relevant supporting entities: a volume contribution on $K\times I_n$, a spatial-facet contribution on $F\times I_n$, and a left temporal-face contribution on $K\times\{t_{n-1}\}$. 
...
The manufactured moving-hill problem was solved on $\Omega=(0,1)^2$ over time interval $[0,1]$, starting from an $n_x \times n_y = 8 \times 8$ mesh in space and $N_t=8$ initial time slabs.
The primal and enriched space–time approximation spaces were \(\mathrm{cG}(1)\otimes\mathrm{dG}(0)\) and \(\mathrm{cG}(2)\otimes\mathrm{dG}(1)\), respectively. Here, \(\mathrm{cG}(p)\) denotes a continuous Galerkin space of polynomial degree \(p\) in space, while \(\mathrm{dG}(q)\) denotes a discontinuous Galerkin space of polynomial degree \(q\) in time.
% All estimators used the same quadrature rule with seven temporal quadrature points.
For the quadratic running goal~\eqref{eq:mh-goal}, the practical two-term nonlinear DWR estimator was used. The D\"orfler marking fraction was set to $\theta=0.40$, the within-slab trigger to $10\%$, and the spatial meshes were refined on each slab. Seven adaptive levels were computed for both methods.
...
% trend, I_eff, DoF
% The relative localisation gap for the bubble automated sequence decreases from \(1.02\%\) to \(0.432\%\), which show that the recovered local contributions reproduce the global estimator closely.

Table~\ref{tab:moving-hill-selected-iterations} shows the results for both residual localisations. Here \(N_{\mathrm{cell}}:=\sum_{n=1}^{N_t}|\mathcal T_n|\) denotes the total number of slabwise space–time cells. During iterations \(0\)--\(2\), both sequences retain \(N_t=8\) and the refinement is predominantly spatial. The small decrease in both effectivity indices suggests that spatial refinement alone does not improve the relative accuracy of the estimator. Once temporal refinement becomes active after iteration \(2\), this imbalance is reduced and both effectivity indices increase steadily towards one, reaching \(0.9941\) and \(0.9955\) at iteration \(6\).

At iteration $6$, both methods reduce the absolute goal error below \(10^{-4}\). The bubble automated method attains a smaller goal error, while the strong manual method uses far fewer space--time cells. 
...
For both localisation methods, refinement follows the trajectory of the moving hill. The principal difference is the spatial distribution of the marking activity. In the strong-residual sequence, a larger fraction of the total activity is concentrated in a relatively small number of cells. Hence, the D\"orfler threshold is reached with fewer marked cells, and the resulting refined patches remain comparatively compact.
