...
We compare with two goal-oriented adaptive strategies: one based on a manually derived residual localisation and one based on the automated bubble--cone recovery developed in \cref{sec:space-time-residual-recovery}.
...
\end{equation}
The factor assigns half of the flux jump to each adjacent cell. For the strongly imposed Dirichlet condition in \eqref{eq:mh-primal}, no additional boundary-facet residual is present.
