...
Let $\Omega=(0,1)^2$, $T=1$, and write $Q:=\Omega\times(0,T)$. We consider the heat equation
\begin{equation}
...
For comparison, we construct a classical non-negative marking score from a manually derived strong-residual representation of the heat equation \cite{Bangerth_Rannacher_2003,Becker_Rannacher_2001}. On a space--time cell \(Q_{K,n}=K\times I_n\), the cell residual is
\begin{equation}
R_{K,n}
:=
...
\end{align}
\end{subequations}
The strong-residual marking score is then
\begin{equation}
m_{K,n}^{\mathrm{str}}
:=
...
\end{equation}
It provides a non-negative measure of the residual on each space--time cell, weighted by its influence on the selected quantity of interest. It discards sign cancellations between residual contributions.
...
The manufactured moving-hill problem was solved on $\Omega=(0,1)^2$ over $[0,1]$, with initial mesh parameter $n_x=n_y=8$, $N_t=8$ initial time slabs, and $\mathrm{CG}(1)\mathrm{dG}(0)$ primal and $\mathrm{CG}(2)\mathrm{dG}(1)$ enriched spaces. All estimators used the same quadrature rule (seven time quadrature points) and the nonlinear DWR identity for the quadratic running goal~\eqref{eq:mh-goal}. The strong-residual bound and the automated localisation both used global bulk marking with $\theta=0.4$, the within-slab trigger $10\%$, and independent slabwise mesh refinement. Both adaptive sequences have nine reported levels. 
...
Both effectivity indices approach one monotonically after time refinement becomes active. The convergence of $I_{\rm eff}$ to one confirms that the DWR identity correctly estimates the quadratic goal error.
...
It is not surprising that the strong-residual method performs slightly better than the automated method in this example. However, the resutls motivate a componentwise marking alternative:
\[
m_{K,n}^{\mathrm{comp}}
:=
|\eta_{\mathrm{vol}}|
