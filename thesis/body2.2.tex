...
The present construction pursues a more geometrically resolved decomposition. It probes the weak residual with polynomial functions supported on individual cells, spatial facets, temporal faces, and, when present, mixed space--time ridges. The resulting coefficients separate the residual mechanisms according to their geometric support, while still being recovered directly from UFL evaluations of the weak residual in \texttt{Firedrake}. Thus no problem-specific strong residual or hand-derived flux-jump formula is needed. This approach is motivated by the stationary bubble--cone construction of Rognes and Logg \cite{Rognes_Logg_2013}, and is extended below to discontinuous Galerkin discretisations in time.


\subsection{Stationary Bubble and Cone Projection}
\label{sec:stationary-residual-recovery}

We first recall the stationary construction for the primal residual. The nonlinear adjoint-residual term is treated in \cref{sec:nonlinear-adjoint-residual-localisation}. For each \(K\in\mathcal T_h\), let \(\mathcal F(K)\) denote its facets. Rognes and Logg formulate the following assumptinos \cite{Rognes_Logg_2013}:
\begin{description}
...
\end{equation}
The bubble test eliminates all facet actions, so \(R_K^B\in\mathbb P^p(K)\) can be recovered from
\begin{equation}
  (R_K^B,b_K\phi)_K
  =\rho_K(b_K\phi)
...
\end{equation}
This cone function is nonzero on the relative interior of \(F\), vanishes on every other facet, and remains nonzero inside \(K\). Let
\(
...
The stationary analysis is established in \cite{Rognes_Logg_2013}, while the space--time extension below is assessed numerically in \cref{chap:experiments} through localisation gaps, effectivities, and adaptive behaviour. The essential new feature is the dG action at each temporal interface.
...
where \(P_{n-1}:V_{n-1}\to V_n\). The transfer and its adjoint are discussed in more details in \cref{sec:slab-transfer-adjoint}.
  ...
  \item[(A2) Local decomposition.] On each space--time cell \(Q_{K,n}\), the residual admits a representation by contributions supported on the cell interior, spatial facets, temporal facets, and, when required by the chosen pairing, their intersections, as shown in \cref{fig:space-time-residual-entities}.
  \begin{equation}
    \begin{aligned}
    \rho_{K,n}(U_h)(V)
    ={}&
...
We first define the temporal interior bubble and its tensor product with the spatial cell bubble from~\cref{eq:recovery-cell-bubble} by
\begin{equation}
  b_n^t(\tau):=4\tau(1-\tau),
  \qquad
  B_{K,n}:=b_Kb_n^t.
...
For the nonlinear identity, the implemented estimator is stated in~\cref{eq:practical-nonlinear-estimator}.
% \begin{equation}
%  \eta
%  =
