...
Figure~\ref{fig:stationary-bubble-cone} illustrates two representative bubble and cone test functions.
...
The stationary analysis is established in \cite{Rognes_Logg_2013}, while the space--time extension below is assessed numerically in Chapter~\ref{chap:experiments} through localisation gaps, effectivities, and adaptive behaviour. The essential new feature is the dG action at each temporal interface.
...
\end{equation}
where \(P_{n-1}:V_{n-1}\to V_n\). The transfer and its adjoint are discussed in more details in Section~\ref{sec:slab-transfer-adjoint}.
