...
The localised DWR contributions defined in \cref{sec:space-time-local-indicator-construction} are associated with space--time cells \(Q_{K,n}=K\times I_n\). We aim to retain the tensor product structure of our space–time mesh, while allowing the spatial mesh to vary from slab to slab. A standard D\"orfler marking strategy, which would select a collection of individual space--time cells for refinement \cite{Dorfler1996}, is not directly compatible with the purpose. 

We therefore apply D\"orfler marking to the space--time entities \((K,n)\), while separating the resulting refinement decisions into spatial and temporal components. For \(\eta_{K,n}\) denoting the signed local DWR contribution associated with \(Q_{K,n}\), we define
\begin{equation}
...
For a prescribed bulk parameter \(\theta\in(0,1]\), the global space--time marking set \(\mathcal M_{\mathrm{st}}\) is chosen as the smallest set of pairs \((K,n)\) satisfying
\begin{equation}
...
\begin{algorithmic}[1]
\Require PDE, initial slab meshes, tolerance \(\mathrm{TOL}\), goal functional \(J\), bulk parameter \(\theta\), temporal threshold \(\varphi_t\)

\While{the stopping criterion is not satisfied}
  \State Construct spatial spaces \(V_n\), enriched spaces, transfer operators \(P_{n}\) and \(P_{n}^{\ast}\).
  \State Solve the primal problem using the dG-in-time formulation \eqref{eq:global-dg-semilinear-form}. Solve the enriched primal problem if required by \eqref{eq:localised-symmetric-nonlinear-dwr}.
  \State Solve the enriched adjoint problems defined by \eqref{eq:discrete-nonlinear-adjoint}. Solve the adjoint problem if required by \eqref{eq:localised-symmetric-nonlinear-dwr}.
  \State Form the adjoint difference \(e_z=z_h^+-z_h\) and the primal difference \(e_u=u_h^+-u_h\). Or form \(z^\star=z_h^+-I_hz_h^+\) if the linear identity is used.
  \State Assemble \(\eta_{\mathrm{global}}\) from \eqref{eq:localised-symmetric-nonlinear-dwr} or \eqref{eq:computable-dwr-interpolated-weight}.
  \State Recover the local residual densities using \eqref{eq:space-time-volume-projection}--\eqref{eq:mixed-ridge-recovery}.
  \State Assemble the entity contributions from \eqref{eq:complete-nonlinear-entity-indicator} and the cell indicators \(\eta_{K,n}\) from \eqref{eq:complete-nonlinear-cell-indicator}.
  ...
  \State Refine the selected spatial meshes and bisect the selected
  temporal slabs.
\EndWhile
\end{algorithmic}
\end{algorithm}
