...
\begin{equation}
  ...
  \qquad
  S\coloneqq\sum_{n=1}^{N}\sum_{K\in\mathcal T_n}s_{K,n}.
...
\pef{Ideally all the adjoint calculations would be automated}
...
The current implementation retains the complete forward trajectory in memory. For longer simulations, Firedrake's automated adjoint derivation supports checkpointing, which stores only selected forward states and recomputes intermediate states during the reverse traversal. This reduces memory consumption at the cost of additional forward computation.
\pef{Cite Griewank \& Walther doi:10.1145/347837.347846}
...
\textcolor{blue}{Again, using pyadjoint could make life easier..? as there will be no need to include $P_{n-1}^{\ast}$. More to discuss after we include NS example}
