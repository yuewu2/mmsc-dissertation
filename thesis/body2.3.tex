...
The default computations use degree-one polynomial densities for the cell, facet, and temporal recovery stages, together with five-point Gauss--Legendre quadrature in time. The resulting global--local discrepancy is monitored through the localisation gap.

In the two-dimensional examples, the initial geometry-conforming meshes and the marked-cell spatial refinements are generated with \texttt{Netgen} and imported through \texttt{Firedrake}'s interface \cite{Schoeberl1997Netgen,Firedrake}.
