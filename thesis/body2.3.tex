...
The localised DWR contributions defined in \cref{sec:space-time-local-indicator-construction} are associated with space--time cells \(Q_{K,n}=K\times I_n\). Direct refinement of the individual space--time cells selected by a standard marking strategy, such as D\"orfler marking~\cite{Dorfler1996}, would generally produce an unstructured space--time mesh. Instead, we retain a tensor-product mesh on each time slab while allowing the spatial mesh to vary between slabs, so that the problem can still be advanced using a standard slabwise time-stepping procedure. We therefore need to devise a specialised marking strategy. 

We separate our refinement decisions into spatial and temporal components. With \(\eta_{K,n}\) denoting the signed local DWR contribution associated with \(Q_{K,n}\), we denote
\begin{equation}
...
We prescribe the D\"orfler marking parameter \(\theta\in(0,1]\), denoting the fraction of the total indicator mass that must be captured by the marked set. The global space--time marking set \(\mathcal M_{\mathrm{st}}\) is then chosen as the smallest set of pairs \((K,n)\) satisfying
\begin{equation}
...
The cells in \(\mathcal{M}_n\) are then marked for spatial refinement, producing
\begin{equation}
...
\end{equation}
A separate temporal refinement decision is then made at slab level. The slab \(I_n\) is bisected when the marked cells constitute at least a prescribed fraction \(\varphi_t\) of its spatial mesh:
\begin{equation}
...
\begin{algorithmic}[1]
\Require PDE residual, goal functional \(J\), initial slab meshes, estimator and enrichment choice, marking parameters \(\theta\) and \(\varphi_t\), and tolerances $\mathrm{TOL}$ and $\mathrm{TOL}_{\mathrm{alg}}$.

\State Initialise the current space--time discretisation

\While{the stopping criterion is not satisfied}
  \State Construct the current and enriched spatial spaces, together with the transfer operators \(P_{n-1}\) and \(P_{n-1}^{\ast}\) defined in \cref{sec:slab-transfer-adjoint}.
  ...
  \Else
  \State Solve the enriched adjoint problem defined by \eqref{eq:enriched-adjoint}. 
  \State Form \(z^\star=z_h^+-\Pi_{h}z_h^+\) according to \eqref{eq:computable-dwr-interpolated-weight}.
...
The default computations use degree-one polynomial densities for the cell, facet, and temporal recovery stages, together with five-point Gauss--Legendre quadrature in time. The resulting global--local discrepancy is monitored at every adaptive iteration through the relative localisation gap. 
% detailed values are reported below only where they are relevant to the interpretation of the results.

In the two-dimensional examples, the initial geometry-conforming meshes and the marked-cell spatial refinements are generated with Netgen and imported through Firedrake's interface \cite{Schoeberl1997Netgen,Firedrake}.
...
The construction above is formulated in terms of nodal interpolation and does not impose physical constraints. For applications involving conserved or divergence-free quantities, however, the transfer \(P_{n-1}\) must be carefully chosen to preserve the relevant physical properties, see, e.g., \cite{non-flow}.
