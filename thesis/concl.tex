...
Second, the structural information produced by the localisation could be used more fully in the marking strategy. The present recovery already distinguishes contributions associated with cell interiors, spatial facets, temporal interfaces, and mixed space--time ridges. However, these contributions are subsequently combined into a single cell--slab score for marking. A possible extension would retain the entity-wise decomposition when deciding whether refinement should be spatial, temporal, or directed towards regions in which several residual mechanisms interact. Such a strategy would require additional numerical validation, particularly in cases with strong cancellation between signed entity contributions.
