...
Finally, the incompressible Navier--Stokes cylinder problem applied the framework to a nonlinear mixed velocity--pressure system. Despite being much more complicated, the same automated localisation applied without modification. The experiment implemented a temporal transfer with a divergence-constrained Stokes projection and demonstrates the practical potential of applying the same localisation framework to a broader range of time-dependent PDE problems.
...
First, the current implementation could be developed into a user-facing software contribution to the Firedrake ecosystem. The aim would be to provide a simple interface through which users specify the weak residual, the quantity of interest, the spatial and temporal discretisations, and the marking parameters. The construction of the adjoint problems, enriched solutions, bubble--cone recovery, local indicators, and slabwise refinement could then be handled automatically by the framework. Such an interface would make the method available to users who want to apply goal-oriented space--time adaptivity without implementing the localisation procedure for each new PDE problem.

Second, more of the structure exposed by the localisation could be retained in the marking strategy. Although the present recovery separates contributions from cell interiors, spatial facets, temporal interfaces, and mixed space--time ridges, the current algorithm combines them into a single cell--slab score. A possible extension would retain this entity-wise decomposition and investigate whether the relative contributions can inform the choice between spatial and temporal refinement.

Rather than inferring the refinement direction from the entity-wise decomposition, another, more direct extension would obtain this information from separate spatial and temporal enrichments.
% A complementary extension would make the enrichment itself more directional. 
% Second, the enrichment and marking strategies could be made more directional. 
Let \(z_{00}\) denote the adjoint in the base space, \(z_{10}\) a spatially enriched adjoint, \(z_{01}\) a temporally enriched adjoint, and \(z_{11}\) an adjoint enriched in both space and time. Define
\[
\delta_h:=z_{10}-z_{00},
\qquad
\delta_k:=z_{01}-z_{00},
\qquad
\delta_{hk}:=z_{11}-z_{10}-z_{01}+z_{00}.
\]
The residual action associated with each component could then be localised separately. For example, the relative magnitudes of
\[
\rho(U_h)(\delta_h),
\qquad
\rho(U_h)(\delta_k),
