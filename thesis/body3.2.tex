...
\end{equation}
% The terminal sensor determines an adjoint trajectory \(x_z(t)=x_d-0.6(1-t)\). It coincides exactly with the primal trajectory when \(x_d=0.8\), while the choices \(x_d=0.7\) and \(0.9\) produce constant offsets of \(-0.1\) and \(+0.1\). 
...
In this numerical test, the initial discretisation contains $12$ spatial cells and $8$ time slabs. The primal solution and numerical dual use cG(1) in space and dG(0) in time. The enriched dual uses the corresponding higher-order space--time approximation. A marking parameter \(\theta=0.60\) is used and a time slab is bisected when the fraction of its marked spatial cells is at least \(15\%\).
