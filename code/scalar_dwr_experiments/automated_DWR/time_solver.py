...
import numpy as np
from firedrake import Constant, Function, Mesh, TestFunction
...
class IrksomeDGSolver:
    ...
    def _initialise_state(
        ...
        if isinstance(initial_data, Function):
            if transfer is None:
                ...
                for bc in self.problem.boundary_conditions(state.function_space()):
                    bc.apply(state)
                return state
            return transfer.adjoint(initial_data, name) if reverse_transfer else transfer.forward(initial_data, name)
    ...
    def _solve_one_slab(
        ...
        *,
        ...
        state = self._initialise_state(
            state, initial_data, transfer, reverse_transfer, f"{name_prefix}_incoming"
        )
        ...
        stepper = TimeStepper(
            ...
            dt,
            state,
            bcs=self.problem.boundary_conditions(V),
    ...
    def solve_terminal_adjoint(
        ...
        if self.problem.requires_primal_linearisation and primal is None:
            raise ValueError("This nonlinear adjoint requires the saved primal slab polynomials.")
        nodes: list[Function | None] = [None] * (nslabs + 1)
        slabs: list[dict[str, Any] | None] = [None] * (nslabs + 1)
        incoming: Function | None = None
        for n in range(nslabs, 0, -1):
            ...
            if self.problem.requires_primal_linearisation:
