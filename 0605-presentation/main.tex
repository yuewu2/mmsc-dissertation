...
In many PDE problems, we do not need the full solution everywhere in space and time.
\vspace{0.3em}
...
The quantity of interest \textbf{encodes why we do the simulation}.
...
\only<1>{
\begin{exampleblock}{Example: fission reactor}
    \small
...
\vspace{0.1em}
\centering
\includegraphics[height=3.5cm]{fission-reactor.png}
}
...
\only<1>{
\node[draw=white, very thick, rounded corners=4pt, fit=(estimate), inner sep=5pt] {};
}
\only<2>{
\node[draw=seabornred, very thick, rounded corners=4pt, fit=(estimate), inner sep=5pt] {};
}
\node[below=3mm of estimate, xshift=-0mm] {\textcolor<1>{white}{\textcolor<2>{seabornred}{challenge}}};
...
% --- Localisation ---
\begin{frame}[t]{Localisation of Error Indicators}

% \small
\textbf{Global estimator.}
\[
\eta \approx \rho(u_h)\bigl(z_h^{(2)}-z_h\bigr).
\]

This estimates the \emph{total} goal error, but does not indicate
where refinement is needed.

\vspace{0.8em}
\pause

\textbf{Stationary implementation.}
\[
\eta \approx \sum_{K\in\mathcal T_h}\eta_K .
\]

Local indicators $\eta_K$ are obtained from residual contributions on
cells and facets, and are used to mark cells for refinement.

\vspace{0.8em}
\pause

\textbf{Time-dependent problems.}
\[
\eta \approx \sum_n \sum_{K\in\mathcal T_h}\eta_{K,n}.
\]
